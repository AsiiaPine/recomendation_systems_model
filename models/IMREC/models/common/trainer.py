...
class Trainer(AbstractTrainer):
    ...
    def _valid_epoch(self, valid_data):
        ...
        # self.eval_metric_dict[self.start_epoch] = valid_score
    ...
    def fit(self, train_data, valid_data=None, test_data=None, saved=False, verbose=True):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            self.eval_metric_dict[epoch_idx] = self.best_valid_result
    ...
    def plot_eval_metric(self, show=True, save_path=None):
        r"""Plot the eval metric in each epoch
        ...
        """
        epochs = list(self.eval_metric_dict.keys())
        epochs.sort()
        eval_metrics_names = list(self.eval_metric_dict[epochs[0]].keys())
        k_values = ['5', '10', '20', '50']

        fig, ax = plt.subplots(ncols=1, nrows=len(k_values), figsize=(10, 6*len(k_values)))


        for i, k in enumerate(k_values):
            metrics_to_plot = [metric_name for metric_name in eval_metrics_names if k in metric_name and not ('50' in metric_name and k != '50')]
            for metric_name in metrics_to_plot:
                values = [float(self.eval_metric_dict[epoch][metric_name]) for epoch in epochs]
                ax[i].plot(epochs, values, label=metric_name)
        ...
        if show:
            plt.show()
        if save_path:
            string = save_path+str(self.best_valid_score)+'.png'
            fig.savefig(string)
        self.logger.info(f'plotting done, {string}')
